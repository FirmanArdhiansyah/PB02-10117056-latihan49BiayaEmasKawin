 ...
 *         Kelas : IF-2 (PB02)
 *         NIM : 10117056
 * Deskripsi Program : Menghitung total Biaya Mahar emas kawin
 */
public class EmasKawin {

    private final double berat;
    private final double hargaPerGram;
...
//     maksimal nilai input -> 2.147.483.647 (untuk tipedata Integer)
    public String terbilang(double i) {
        ...
        if (j < 12) {
            ...
            return angka[j - 10] + "belas ";
        } else if (j < 100) {
            return angka[j / 10] + "puluh " + terbilang(j % 10);
        } else if (j < 200) {
            return "seratus " + terbilang(i - 100);
        } else if (j < 1000) {
            return angka[j / 100] + "ratus " + terbilang(j % 100);
        } else if (j < 2000) {
            return "seribu " + terbilang(i - 1000);
        } else if (j < 1000000) {
            return terbilang(j / 1000) + "ribu " + terbilang(j % 1000);
        } else if (j < 1000000000) {
            return terbilang(j / 1000000) + "juta " + terbilang(j % 1000000);
        } else if (j <= 2147483647) {
            return terbilang(j / 1000000000) + "miliar " + terbilang(j % 1000000000);
        }
        return angka[j];
    }
